  ...
  */
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      ...
      try {
        const response = await axios.get(`https://tembea.onrender.com/api/wishlist/user/${user.id}`);
        const productIds = response.data.wishlist.map((item: any) => item.product_id);
  
        const productDetails = await Promise.all(
          productIds.map(async (productId: number) => {
            const productResponse = await axios.get(`https://tembea.onrender.com/api/tours/${productId}`);
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://tembea.onrender.com/api/bookings/user/${user.id}`);
        ...
        const enrichedBookings = await Promise.all(
          bookingIds.map(async (booking: any) => {
            const bookingResponse = await axios.get(`https://tembea.onrender.com/api/bookings/${booking}`);
  ...
  const removeFromWishlist = async (userId: number, productId: number) => {
    try {
      const response = await axios.delete("https://tembea.onrender.com/api/wishlist/remove", {
        data: {
          user_id: userId,
